...
def create_windows(pattern, mask):
    ...
    if dims == 3:
        ...
        base_penalty[np.where(mask1)] = 1.0
...
def diffpoisson(scale, y, basey, minval, absrefpoint, filter, qbarrier):
    ...
    f = lambda x, grad=0: diff_func(scale, rscale, mask, y, baseyscaled, np.asarray(minval), absrefpointscaled, filterrsq, qbarrier,
                                    x, grad)
...
def diff_func(scale, rscale, mask, y, base_y, minval, absrefpoint, filterrsq, qbarrier, x, grad=False, ret_vals=False):
    ...
    vals[mask] = np.asarray(-(y[mask] * np.log((x_upperlim[mask] + base_y[mask]) / np.maximum(absrefpointupperlim[mask], 0.5e-9))
